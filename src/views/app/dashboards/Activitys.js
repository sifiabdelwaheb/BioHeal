...
import ActivityForms from "../../../common/Activity";
...
function Activitys(props) {
  ...
  const [ActivityDetails, setActivityDetails] = useState(ActivityForms());
  ...
  const onSendFormHandler1 = () => {
    setClick(true);
    if (isValid) {
      OnGenerate(dataObj(ActivityDetails));
    }
  };
      ...
      >
        <>
          <Card
            ...
            sm="6"
            md="12"
            package={"Add new treatments or Activities "}
            withImgCard={false}
          ...
          >
            ...
            {!redux.foods.loaded && clicked && !redux.foods.error ? (
...
export default injectIntl(Activitys);
